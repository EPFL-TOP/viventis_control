...
from bokeh.models import ColumnDataSource, BoxEditTool, TapTool, LabelSet, Button, CheckboxGroup, TextInput, Div, Range1d, Slider, Select
...
p.image('image', x='x', y='y', dw='dw', dh='dh', source=image_source, palette="Greys256")
...
slider = Slider(start=0, end=0, value=0, step=1, title="Time Point", width=250)



#___________________________________________________________________________________________
def select_y_range(attr, old, new):
    print('checkbox_yrange.active  ',checkbox_maxproj.active)
...
def fill_source_image():
        ...
        image_max_source.data = {'image':[np.flip(np.max(arr, axis=0),0)], 'x':[0], 'y':[0], 'dw':[intensity_normalized.shape[1]],'dh':[intensity_normalized.shape[0]]}
...
layout = column(select_button, row(p,column(checkbox_maxproj,dropdown_downscale)), slider_layout, controls, status)
